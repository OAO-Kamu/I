...
local Window = Library:CreateWindow{
    Title = "打断过你的骨头",
    SubTitle = "By Fatigue 汉化: BH Team-Q3E4",
    TabWidth = 160,
...
-- Autofarm toggle
local Toggle = Tabs.Main:CreateToggle("AutofarmToggle",{Title="自动刷钱",Default=false}):OnChanged(function(v)
    runningAutofarm = v
    if v then task.spawn(autofarmLoop) end
end)

-- Slam Duration
local Slider = Tabs.Main:CreateSlider("SlamDuration",{Title="持续时间",Min=5,Max=60,Default=20,Rounding=1}):OnChanged(function(val)
    slamDuration = val
end)

-- Slam Velocity
local Slider = Tabs.Main:CreateSlider("SlamVelocity",{Title="刷钱速度",Min=50,Max=500,Default=100,Rounding=1}):OnChanged(function(val)
    slamVelocity = val
end)

-- Auto Upgrade Bones
local Toggle = Tabs.Main:CreateToggle("AutoUpgrade",{Title="自动升级骨头",Default=false}):OnChanged(function(v)
    runningUpgrade = v
    if v then task.spawn(autoUpgradeLoop) end
end)

-- Auto Next Ragdoll
local Toggle = Tabs.Main:CreateToggle("AutoNextRagdoll",{Title="自动买下一个区域",Default=false}):OnChanged(function(v)
    runningNextRagdoll = v
    if v then task.spawn(autoNextRagdollLoop) end
end)

-- Auto Refine
local Toggle = Tabs.Main:CreateToggle("AutoRefine",{Title="自动买新身体",Default=false}):OnChanged(function(v)
    runningRefine = v
