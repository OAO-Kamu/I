...
local Luna = { 
	Folder = "Luna", 
	Options = {}, 
	ThemeGradient = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(255, 102, 255)), ColorSequenceKeypoint.new(0.50, Color3.fromRGB(123, 201, 201)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(224, 138, 175))} 
}
...
local function Hide(Window, bind, notif)
	...
	bind = bind[2]
	if notif then
		Luna:Notification({Title = "主页面已隐藏!", Content = "按键盘:"..tostring(bind).."键打开", Icon = "visibility_off"})
	end
...
function Luna:CreateWindow(WindowSettings)
	...
	function Window:CreateHomeTab(HomeTabSettings)
		...
		HomeTabSettings = Kwargify({
			...
			DiscordInvite = "https://discord.gg/QE6gZXpk" 
		}, HomeTabSettings or {})
		...
		HomeTabPage.detailsholder.dashboard.Client.Title.Text = (isStudio and "你可能在: Roblox Studio" or identifyexecutor()) or "你的注入器不支持显示!"
		for i,v in pairs(HomeTabSettings.SupportedExecutors) do
			if isStudio then HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "Luna 接口调试模式(测试)" break end
			if v == identifyexecutor() then
				HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "你的注入器支持这个脚本!"
				break
			else
				HomeTabPage.detailsholder.dashboard.Client.Subtitle.Text = "你的注入器可能不是官方版!"
				break
			end
		end
		...
		local function checkFriends()
			if friendsCooldown == 0 then
				...
				HomeTabPage.detailsholder.dashboard.Friends.All.Value.Text = tostring(friendsInTotal).." 好友"
				HomeTabPage.detailsholder.dashboard.Friends.Offline.Value.Text = tostring(friendsInTotal - onlineFriends).." 好友"
				HomeTabPage.detailsholder.dashboard.Friends.Online.Value.Text = tostring(onlineFriends).." 好友"
				HomeTabPage.detailsholder.dashboard.Friends.InGame.Value.Text = tostring(friendsInGame).." 好友"
		...
		coroutine.wrap(function()
			while task.wait() do
				...
				-- Players
				HomeTabPage.detailsholder.dashboard.Server.Players.Value.Text = #Players:GetPlayers().." 正在玩"
				HomeTabPage.detailsholder.dashboard.Server.MaxPlayers.Value.Text = Players.MaxPlayers.." 最多玩家"

				-- Ping
				HomeTabPage.detailsholder.dashboard.Server.Latency.Value.Text = isStudio and tostring(math.round((Players.LocalPlayer:GetNetworkPing() * 2 ) / 0.01)) .."毫秒" or tostring(math.floor(getPing()) .."毫秒")
